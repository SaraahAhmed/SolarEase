...
namespace SolarEase.Controllers
{
    ...
    public class PersonsController : ControllerBase
    {
        ...
        // update person system size
        [HttpPut]
        [Route("UpdateSystemSize/{id:int}")]
        //[Authorize]
        public async Task<IActionResult> UpdateSystemSize([FromRoute] int id, [FromForm] UpdatePersonSystemSizeDto updatePersonSystemSizeDto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            //Update all parameters
            var personModel = updatePersonSystemSizeDto.PersonUpdateSystemSize();

            //update database
            personModel = await personRepository.UpdateSystemSizeAsync(id, personModel);
            if (personModel == null)
            {
                return NotFound();
            }
